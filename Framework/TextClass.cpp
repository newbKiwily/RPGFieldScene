...
TextClass::TextClass(int x, int y) :posX(x), posY(y)
{
	m_Font = 0;
	m_FontShader = 0;
	...
	float red, float green, float blue, ID3D11DeviceContext* deviceContext)
{
	...
	drawX = (float)(posX - (m_screenWidth / 2));
	...
	XMMATRIX worldMatrix, XMMATRIX orthoMatrix)
{
	...
	result = m_FontShader->Render(deviceContext, sentence->indexCount, worldMatrix, XMMatrixIdentity(),
