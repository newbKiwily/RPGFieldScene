...
SkinModel* ModelLoader::LoadModel(CString path, UINT flag)
{
	...
	if (!model->CreateModel(D3DClass::GetInstance().GetDevice(), vertices, indices)) {
		SAFE_RELEASE(model);
		return NULL;
	}
...
bool IsAssimpHelperNode(const wstring& name)
{
	return
		name.find(L"$AssimpFbx$_Translation") != wstring::npos ||
		name.find(L"$AssimpFbx$_Rotation") != wstring::npos ||
		name.find(L"$AssimpFbx$_Scaling") != wstring::npos ||
		name.find(L"$AssimpFbx$_PreRotation") != wstring::npos ||
		name.find(L"$AssimpFbx$_PostRotation") != wstring::npos ||
		name.find(L"$AssimpFbx$_Transform") != wstring::npos;
}
void ModelLoader::ProcessNode(aiNode* aiNodeInfo, SkinModel* skModel, NodeInfo* parent, int depth)
{
	...
	{
		// 단, transform은 parent에 곱해줘야 한다
		XMMATRIX helperTM = XMMatrixTranspose(XMMATRIX(aiNodeInfo->mTransformation[0]));

		if (parent)
			parent->localTM = helperTM * parent->localTM;

		// children을 parent 아래로 그대로 이어붙임
		for (UINT i = 0; i < aiNodeInfo->mNumChildren; i++)
		{
			this->ProcessNode(aiNodeInfo->mChildren[i], skModel, parent, depth);
		}
		return;
	}
	...
	// 로그 출력 (필요하다면 유지)
	wstring indent(depth * 2, L' ');
	wstring log = L"[Bone Hierarchy] " + indent + L"└ " + nodeName + L"\n";
