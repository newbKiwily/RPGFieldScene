...
//셰이더를 추가한다.
bool Shader::InsertShader(ID3D11Device* device, HWND hwnd, WCHAR* fileName)
{
	//셰이더 컴파일 및 초기화
	if (!InitializeShader(device, hwnd, fileName)) {
		return false;
	}
...
//셰이더를 읽어 컴파일 후 생성
bool Shader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* fileName)
{
	...
	if (FAILED(D3DCompileFromFile(fileName, NULL, NULL, "vsMain", "vs_5_0",
		D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage))) {
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage) {
			OutputShaderErrorMessage(errorMessage.Get(), hwnd, fileName);
		}
	...
	if (FAILED(D3DCompileFromFile(fileName, NULL, NULL, "psMain", "ps_5_0",
	...
	BuildLayoutDesc(CT2CA(fileName), layoutDescInfo);
...
int Shader::FindLayout(vector<D3D11_INPUT_ELEMENT_DESC>& refLayoutDesc)
{
	...
	for (unsigned int i = 0; i < _layoutList.size(); i++) {
