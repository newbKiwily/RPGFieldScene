...
void FontClass::BuildVertexArray(void* vertices, const char* sentence, float drawX, float drawY)
{
	...
	for (i = 0; i < numLetters; i++)
	{
		...
		else
		{
			...
			vertexPtr[index].position = XMFLOAT3(drawX, drawY, 1.0f);  // Top left.
			vertexPtr[index].texture = XMFLOAT2(m_Font[letter].left, 0.0f);
			index++;

			vertexPtr[index].position = XMFLOAT3((drawX + m_Font[letter].size), (drawY - 16), 1.0f);  // Bottom right.
			vertexPtr[index].texture = XMFLOAT2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = XMFLOAT3(drawX, (drawY - 16), 1.0f);  // Bottom left.
			...
			vertexPtr[index].position = XMFLOAT3(drawX + m_Font[letter].size, drawY, 1.0f);  // Top right.
