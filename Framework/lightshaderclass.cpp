...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./data/phongLightShader2.hlsl");
	if(!result)
	{
		return false;
	}
	...
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower,XMFLOAT4 pointLightColor[], XMFLOAT4 pointLightPosition[])
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureArray,
		lightDirection, ambientColor, diffuseColor, cameraPosition, specularColor, specularPower,pointLightColor,pointLightPosition);
	if(!result)
	{
		return false;
	}
...
void LightShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	...
	// Pop a message up on the 
	// to notify the user to check the text file for compile errors.
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
	...
	XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower, XMFLOAT4 pointLightColor[], XMFLOAT4 pointLightPosition[])
{
	HRESULT result;
	...
	PointLightColorBufferType* dataPtr4;
	PointLightPositionBufferType* dataPtr5;
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr4 = (PointLightColorBufferType*)mappedResource.pData;

	// Copy the light color variables into the constant buffer.
	dataPtr4->diffuseColor[0] = pointLightColor[0];
	dataPtr4->diffuseColor[1] = pointLightColor[1];
	dataPtr4->diffuseColor[2] = pointLightColor[2];
	dataPtr4->diffuseColor[3] = pointLightColor[3];
	dataPtr4->diffuseColor[4] = pointLightColor[4];
	dataPtr4->diffuseColor[5] = pointLightColor[5];
	dataPtr4->diffuseColor[6] = pointLightColor[6];
	dataPtr4->diffuseColor[7] = pointLightColor[7];
	// Unlock the constant buffer.
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr5 = (PointLightPositionBufferType*)mappedResource.pData;

	// Copy the light position variables into the constant buffer.
	dataPtr5->lightPosition[0] = pointLightPosition[0];
	dataPtr5->lightPosition[1] = pointLightPosition[1];
	dataPtr5->lightPosition[2] = pointLightPosition[2];
	dataPtr5->lightPosition[3] = pointLightPosition[3];
	dataPtr5->lightPosition[4] = pointLightPosition[4];
	dataPtr5->lightPosition[5] = pointLightPosition[5];
	dataPtr5->lightPosition[6] = pointLightPosition[6];
	dataPtr5->lightPosition[7] = pointLightPosition[7];
	// Unlock the constant buffer.
